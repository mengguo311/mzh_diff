...
> 撰稿口径：本报告截至 **v13 A1（缩窗 L=512）**主力版本完成、**v13 C1（富条件 + 自回归）**评估进行中。所有统计量取自项目 eval 量具实测与诊断图，关键数字均在表中【加粗】标注。图片以相对路径 `outputs/figures/summary_v13/<文件名>` 内嵌（本报告位于 `/home/u00134/src/`）。
...
**v13 进展。** A1 把窗从 2048 缩到 **512**（独立段 7.2→28.8，×4，零新数据 / 零合成 / 零失真），机制性把复制率压到史上最低 **14.5%**、C2ST_新颖压到史上最优 **0.722**；**但代价是签名律塌（SigP_新颖 0.003）与长程结构塌陷**（拼回 2048 后 regime 持续期仍 44 vs 真实 33、std 萎缩 0.0077 vs 0.0105、高波动占比 gap −0.24）。C1 富条件自回归试图修补长程，**训练已完成、长程评估进行中**，初步发现自回归 2048 输出**数值发散**（std≈0.105 / 峰度≈40026），疑似误差累积或逆变换问题，已从定量对比剔除。

**一句话结论。** 在 ~7 个独立宏观窗的硬数据约束下，"既新颖又逼真"的金融序列生成**尚无解**；模型侧（损失 / 架构 / 采样 / 正则 / 缩窗）只是在同一堵 58M/7 的墙上挪动 Pareto 前沿——**治本唯一方向是数据**（不失真地增加独立信号）。
...
**v13 C1 富条件 + 自回归（进行中，数值不稳定）**：用 16 维富上下文链式自回归拼回 2048 修补长程。**训练已完成、长程评估进行中、定量对比剔除**。初步自回归输出**数值发散**（部分样本 std≈0.105 / 峰度≈40026，DGS10 出现 max|·|>0.5 异常），疑似误差累积 / 接缝 / 逆变换问题，待排查。
...
| v13 C1 富条件+自回归 | COND_DIM=16 + 链式 512→2048 | — | — | — | — | **评估进行中,定量剔除**;初步自回归输出**数值发散**(std≈0.105/峰度≈40026)待排查 |
...
### 8.1 v13 C1（富条件 + 自回归，进行中）

- **状态**：训练已完成、长程评估进行中、定量对比剔除。
- **机制**：`USE_CONTEXT_COND=True`（COND_DIM=16，16 维富上下文：std/|r|均值/均值/|r|-acf1/skew/kurt/末值/d2能量 ×2 通道）；`generate_autoregressive.py` 链式 512→2048（k=4），每窗以前一窗 `ctx_features(x0).detach()` 为条件，朴素拼接、严禁真实数据填缝，含 `--force_null` 消融臂。
- **初步问题**：自回归 2048 输出**数值发散**——部分样本 std≈**0.105**（真实 0.0105 的 10 倍）、峰度≈**40026**，DGS10 出现 max|·|>0.5 异常值；首几个 chunk 仍稳定（std 0.0105）。
- **待排查**：是**误差累积**（自回归把每窗微小偏差逐窗放大）还是**接缝 / 逆变换问题**。需先过 context 消融门（zero-ctx vs real-ctx 的 regime KS，无差异即停），并解决发散后才能定量对比；否则退回 A2 配置。
...
模型侧已到极限，治本路线全部指向**不失真地增加独立信号**：缩窗（Step，已做）→ block-bootstrap（A2 备件）→ 多资产同构对预训练（B1）→ 富条件自回归长程修补（C1，需先解决发散）→ 更长历史 / 更多资产数据。详见 `~/src/eval/docs/v13_data_plan.md`。
...
| **真实 SP500 超额峰度** | **18.80**（全集）/ 21.80（3000样本） | v9 4.71 / v10重训 7.66 / v13_a1 8.63（均仅约 1/3–1/2 真实） |
| **真实 SP500 偏度** | **−0.679** | 生成 −0.11 ~ −0.23（欠左偏） |
| **真实 \|r\| ACF1** | **0.263** | v10重训 0.212 / v13_a1 **0.135**（最弱波动聚集） |
| **复制率底噪（real-real）** | **0.0%** | — |
| **复制率（越低越好）** | 0.0% | v9 30.1% / v10重训 **53.9%（最差）** / v13_a1 **14.5%（史上最低）** |
| **C2ST_新颖（越接近标定越好）** | L512 **0.4965** / L2048 0.495 | v10重训 0.821 / v13_a1 **0.722（史上最优）** |
| **SigP_新颖（越高越好）** | L2048 **0.326** / L512 **0.116** | v10重训 **0.585（唯一入标定带）** / v13_a1 0.003（塌） |
...
| **v13 C1 自回归发散** | 真实 std 0.0105 | 发散样本 std≈**0.105** / 峰度≈**40026**（待排查） |
