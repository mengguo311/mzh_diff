...
|---|---|---|---|---|
| **line1_clip20(line1)🔥** | **0.770** | **0.680** | **90.3%** | **9.7%** |
| **line1_clip20_acf(line1 收官)🔥** | **0.772** | **0.679** | **90.2%** | **9.8%** |
| **line1_clip15(line1)🔥** | **0.763** | **0.673** | **90.2%** | **9.8%** |
...
> **🔥 line1 真实度模型补评(2026-06-26,15 模型同口径:test_AUC 0.9998 / 阈值 0.5064 / 真实基线 p_real 0.643、judged_real 100%,与上表可比)**:`output/line1/l1_clip{15,20,20_acf}.csv`(各 1024 条)评分 **fool 全部 ~90.2-90.3%**,**居 15 模型之首**,远超 v9/v10(63-68%)、甚至高于真实窗自身 p_real(0.643)。
> **⚠️ 这部分推翻本节旧"越先进越易被鉴别"结论**:line1 模型【既真·真实(峰度~20 真厚尾、realism_board 4族PASS)又骗过 hw01 90%】,**反例**了"真实度↔fool 必冲突"。
> **机制**:line1 补了 **DGS10 量化吸附 0.01**(`DGS10_QUANTIZE`,真·真实属性——真实债券差分按 basis point 量化),正好**拆掉 hw01 最强的确定性抓手**(旧 12 模型全连续浮点 rounding≈0 被硬抓;line1 rounding≈1.0 与真实一致)→ p_real 分类器失去该轴 + 风格事实概率升 → fool 飙到 90%。**即"匹配真实的量化指纹"同时兑现真实度与骗过外部鉴别器**,二者并非天然冲突,旧结论的冲突源于此前无人补 DGS 量化。**建议**:给项目内部诚实闸门也补 DGS10 量化检测(否则内外都漏判"未量化=假"这一确定性信号)。
